...
public class EditorManager : BaseBehavior
{
  ...
  //  public CustomButton rotateRight;
    ...
    public AutoCompleteComboBox loadDropdown;
    ...
    private static string dropDownListItem = "";
 ...
 //       rotateLeft.onClick.AddListener(() => this.RotateSceneLeft());
   //     rotateRight.onClick.AddListener(() => this.RotateSceneRight());
        ...
        {
            this.loadDropdown.AddItem(file);
        }
       ...
       if(dropDownListItem.Length != 0)
            this.loadDropdown.setText(dropDownListItem);
    }
/*
    private void RotateSceneRight()
    {
        RotateScene(90, -1);

    }

    private void RotateSceneLeft()
    {
        RotateScene(90, 1);
    }

    private void RotateScene(int rotation, int side)
    {
        foreach (var aa in  warehouseManager.ActiveData.activeItems)
        {
            aa.transform.RotateAround(transform.position + new Vector3(1f, 1f, 0f), Vector3.forward, rotation*side);
        }
        
        warehouseManager.groundHolder.transform.RotateAround(transform.position + new Vector3(1f, 1f, 0f), Vector3.forward, rotation*side);
        // warehouseManager.itemHolder.transform.RotateAround(transform.position + new Vector3(1f, 1f, 0f), Vector3.forward, rotation*side);
    }
*/
    public void LoadAndReplace(bool changed)
    {
        string relativePath = this.loadDropdown.Text;
        if (changed && !dropDownListItem.Equals(relativePath))
        {
   ...
   // private void OnPanelSelection(UiSpritePanel panel, string buttonText) => this.OnPanelSelection(panel, null, buttonText);
    ...
    private void SaveSettings()
    {
        ...
        if (this.loadDropdown.Text != null)
        {
            data.loadName = this.loadDropdown.Text;
        }
    }
    ...
    private void ExportActive()
    {
        ...
        string textFile = bundle.PostfixString;
        if (this.loadDropdown.Text != null)
        {
            textFile = this.loadDropdown.Text;
        }
        var result = this.File.Save(bundle.levelData, false, textFile, false);
        if (result.IsValid())
        {
            dropDownListItem = result.Replace(".json", "");
            this.loadDropdown.AddItem(dropDownListItem);
            this.loadDropdown.setText(dropDownListItem);
        }
