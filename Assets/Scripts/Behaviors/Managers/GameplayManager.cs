        ...
        this.alert.Prompt(result =>
        {
            switch (result.result)
            {
                case Alert.ResultType.Accepted:
                    ...
                    int numScene = filePaths.IndexOf(this.Context.data.activeWarehouseLevelMeta.relativePath)+1;
                    var fileData = filePaths[numScene];
                    var data = this.File.LoadJson<WarehouseManager.Data>(fileData);
                    this.Context.data.SetActiveData(this.Context, data, fileData);
