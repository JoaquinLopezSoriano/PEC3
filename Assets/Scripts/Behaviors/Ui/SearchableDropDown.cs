...
public class SearchableDropDown : MonoBehaviour
{
    ...
    private List<Toggle> initializedButtons = new List<Toggle>();
    ...
    public void AddItemToScrollRect(List<string> options)
    {
        ...
        {
            GameObject buttObj = Instantiate(buttonsPrefab, content);
            buttObj.GetComponentInChildren<Text>().text = option;
            buttObj.name = option;
            buttObj.SetActive(true);
            Toggle butt = buttObj.GetComponent<Toggle>();
            butt.onValueChanged.AddListener(delegate { OnItemSelected(buttObj); });
